...
-- [실행 1/1] inserting dummy data to ssf_user
SELECT CONCAT(
    'INSERT INTO ssf_user (user_key, email, username, userpwd, banned, signout, signin, snsprov, snsid, referralId) VALUES (\'',
    user_key, '\', \'',
    email, '\', \'',
    username, '\', \'',
    userpwd, '\', \'',
    banned, '\', ',
    IFNULL(CONCAT('\'', signout, '\''), 'NULL'), ', ',
    '\"', signin, '\", ',
    IFNULL(CONCAT('\'', snsprov, '\''), 'NULL'), ', ',
    IFNULL(CONCAT('\'', snsid, '\''), 'NULL'), ', ',
    IFNULL(CONCAT('\'', referralId, '\''), 'NULL'),
    ');'
) AS insert_stmt
FROM ssf_user;
